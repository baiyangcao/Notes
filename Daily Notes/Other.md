...
```
 ...
 ## ArcSDE 服务启动报错 `Error 1503`

 今天晨晨姐安装 ArcSDE 10.1 for Oracle 11g ，到最后启动服务的时候报错：

 ```
 Error 1503：服务没有及时响应启动或控制请求
 ```

因为服务器上安装的 64 位的 ArcSDE ，但是安装的 Oracle 客户端是 32 位的，
导致 ArcSDE 连接 Oracle 数据库的时候报错，导致服务无法启动。  
